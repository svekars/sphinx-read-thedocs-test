...
async def main():
    ...
    # Get list of RST files from stdin
    rst_files = []
    excluded_dirs = {'_templates', '_static', 'generated', 'build'}
    
    for line in sys.stdin:
        path = Path(line.strip())
        # Check if any part of the path contains excluded directories
        if not any(excluded_dir in part for part in path.parts for excluded_dir in excluded_dirs):
            rst_files.append(path)
    
    if not rst_files:
        print("No RST files found in input (or all files were in excluded directories)", file=sys.stderr)
        sys.exit(1)
    
    total_files = len(rst_files)
    print(f"Found {total_files} total RST files (excluding _templates and _static directories)")
